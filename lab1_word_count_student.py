...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = len(wordsGrouped.map(lambda (k, v): k).collect())
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    output = wordListRDD.map(lambda word: (word, 1)).groupByKey().map(lambda (k,v): (k, sum(v)))
    return output      
...
def removePunctuation(text):
    ...
    """
    output =re.sub(r'[^0-9a-z ]+', "", text.lower()).lstrip().rstrip()
    return output
...
#output = shakespeareWordsRDD.flatMap(lambda x: x.split())
#print shakespeareWordsRDD
