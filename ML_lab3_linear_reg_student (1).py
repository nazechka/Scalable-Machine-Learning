...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    splitted = unicode.split(line,',')
    label, features = splitted[0], splitted[1:]
    return LabeledPoint(label, features)
    
print parsePoint(samplePoints[0]) 
...
#print parsedDataInit.collect()
onlyLabels = parsedDataInit.map(lambda lp: lp.label).collect()
#print onlyLabels
...
# TODO: Replace <FILL IN> with appropriate code
#parsedData = parsedDataInit.map(lambda lp: (lp.label - 1922, lp.features))
parsedData = parsedDataInit.map(lambda lp: LabeledPoint(lp.label-1922, lp.features))
#print parsedData
...
def calcRMSE(labelsAndPreds):
    ...
    """
    value = labelsAndPreds.map(lambda (x, y): squaredError(x,y)).sum()
    return math.sqrt(value/len(labelsAndPreds.collect()))
...
def gradientSummand(weights, lp):
    ...
    """
    value = (weights.dot(lp.features)-lp.label)*lp.features
    return value
...
#labelsAndPredsExample = predictionExample.map(lambda lp: gradientSummand(DenseVector(weights), lp))
...
#exampleErrorTrain = linregGradientDescent(exampleData, exampleNumIters)
...
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations=numIters, step=alpha,
        regParam=reg, miniBatchFraction=miniBatchFrac,regType=regType, intercept=True)
...
samplePrediction = firstModel.predict(parsedTrainData.take(1)[0].features)
...
def product((x,y)):
    return x*y
res= [product(item) for item in list(itertools.product([2,3],[2,3]))]
print res
...
#    """
    results = [product(item) for item in list((itertools.product(lp.features, lp.features)))]
    #lp.features = list(lp.features).append(results)
    lp.features = np.hstack((lp.features, results))
    return LabeledPoint(lp.label, lp.features)                  
