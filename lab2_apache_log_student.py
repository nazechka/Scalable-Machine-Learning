...
# This was originally '^([\S]+[\-]*) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+) (\S+)\s*(\S*)\s" (\d{3}) (\S+)'
...
uniqueHostCount = len(uniqueHosts.map(lambda (k,v): k).collect())
...
dayGroupedHosts = dayToHostPairTuple.groupByKey().map(lambda (a,b): (a, len(set(b))))
#print dayGroupedHosts.collect()
dayHostCount = dayGroupedHosts.reduceByKey(lambda a, b: a+b).cache()
...
groupedByDay = dayAndHostTuple.groupByKey().cache()
sortedByDay = groupedByDay.sortByKey()

avgDailyReqPerHost = (sortedByDay.map(lambda (a,b): (a, int((len(b))/(len(set(b))))))).cache()
